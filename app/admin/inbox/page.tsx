...
export default async function InboxPage(props: PageProps<"/admin/inbox">) {
  // Admin-only, and no layout above this one enforces it — /admin/inbox sits outside the
      ...
      <nav className="mb-6 flex gap-1 border-b border-border">
        {TABS.map((entry) => {
          ...
          return (
            <Link
              key={entry.key}
              href={`/admin/inbox?tab=${entry.key}`}
            ...
            <article key={submission.id} className="flex flex-col gap-3 py-5">
              ...
              {known && !submission.promotedAt && (
                <p className="text-[11px] text-warning">
                  Already a lead —{" "}
                  <Link href={`/admin/leads/${known.id}`} className="underline">
                    {known.name}
                  </Link>
              ...
              {submission.promotedAt && submission.promotedContact && (
                <p className="text-[11px] text-success">
                  Added to leads as{" "}
                  <Link href={`/admin/leads/${submission.promotedContact.id}`} className="underline">
